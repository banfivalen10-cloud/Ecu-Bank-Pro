    ...
    return (
      <div className="min-h-screen flex flex-col bg-[#050811] text-slate-100">
        ...
        <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10 flex-1 w-full">
          ...
          <section className="mt-6 rounded-2xl border border-white/10 bg-slate-900/60 p-5 sm:p-6"><div className="mb-5"><div className="text-[10px] uppercase tracking-widest text-cyan-300 font-bold">SOFTWARE INCLUIDO</div><h2 className="text-xl font-bold text-white mt-1">Programas y enlaces del paquete</h2></div><div className="grid sm:grid-cols-2 gap-3">{[
          ...
          ].map((item) => <div key={item.title} className="p-4 rounded-xl border border-white/10 bg-slate-950/60 flex items-center justify-between gap-3"><div className="min-w-0"><h3 className="font-semibold text-white text-sm">{item.title}</h3><p className="text-xs text-slate-400 mt-1">{item.subtitle}</p></div><Button size="sm" variant="outline" onClick={() => isVip ? window.open(item.link, "_blank", "noopener,noreferrer") : setLocation("/pricing")} className="border-fuchsia-400/30 text-fuchsia-300 shrink-0">{isVip ? <><ExternalLink className="h-3.5 w-3.5 mr-1.5" /> Abrir</> : <><Lock className="h-3.5 w-3.5 mr-1.5" /> VIP</>}</Button></div>)}</div></section>
