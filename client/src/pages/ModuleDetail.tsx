    ...
    return (
      <div className="min-h-screen flex flex-col bg-[#050811] text-slate-100">
        ...
        <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10 flex-1 w-full">
          ...
          <section className="mt-6 rounded-2xl border border-sky-400/25 bg-sky-950/20 p-5 sm:p-6"><div className="mb-4"><div className="text-[10px] uppercase tracking-widest text-sky-300 font-bold">ISAAC REPARADOR AUTOMOTIVO</div><h2 className="text-xl font-bold text-white mt-1">Empieza por aquí ⬇️</h2></div><div className="grid sm:grid-cols-2 gap-3">{["Link 1 — SPC MAX MX-0017", "Link 2 — Autocom 2021.11", "Link 3 — Delphi 2017 R3", "Link 4 — WOW", "Link 5 — Info"].map((item) => <div key={item} className="flex items-center justify-between gap-3 rounded-lg border border-white/10 bg-slate-950/60 p-3"><span className="text-sm text-slate-200">{item}</span><Button size="sm" variant="outline" onClick={() => toast.info("Link pendiente", { description: "Pasame el enlace de este recurso y lo conecto." })} className="border-sky-400/30 text-sky-300 shrink-0">Pendiente</Button></div>)}</div></section>
