...
export default function ModuleDetail() {
  ...
  const openEcmDownload = () => {
    if (!isVip) {
      toast.error("Descarga protegida", { description: "Necesitas una membresía VIP activa para descargar ECM Titanium.", action: { label: "Ver planes", onClick: () => setLocation("/pricing") } });
      return;
    }
    window.open(ECM_MEGA_URL, "_blank", "noopener,noreferrer");
  };

  const openEcmFile = (file: (typeof ecmTitaniumTree.children)[number]) => {
    if (!isVip) {
      toast.error("Archivo protegido", { description: "Necesitas una membresía VIP activa para abrir los archivos de ECM Titanium.", action: { label: "Ver planes", onClick: () => setLocation("/pricing") } });
      return;
    }
    window.open(file.webViewLink, "_blank", "noopener,noreferrer");
  };
