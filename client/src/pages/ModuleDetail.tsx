...
export default function ModuleDetail() {
  ...
  const [bonusPassword, setBonusPassword] = useState("");
  ...
  const mod = data?.module ?? FALLBACK_MODULES.find((item) => item.id === moduleId);
  ...
  if (moduleId === "bonus") {
    return (
      <div className="min-h-screen flex flex-col bg-[#050811] text-slate-100"><Navbar /><main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10 flex-1 w-full"><Link href="/" className="inline-flex items-center gap-2 text-xs font-semibold text-slate-400 hover:text-white"><ArrowLeft className="h-3.5 w-3.5" /> Volver a los módulos</Link><div className="mt-10 p-6 sm:p-10 rounded-3xl border border-fuchsia-500/30 bg-gradient-to-br from-fuchsia-950/40 via-slate-900/80 to-slate-950 text-center shadow-2xl shadow-fuchsia-950/20"><div className="inline-flex items-center gap-2 px-3 py-1 rounded-xl border border-fuchsia-400/40 bg-fuchsia-500/10 text-fuchsia-300 text-[11px] font-bold uppercase tracking-wider"><Key className="h-4 w-4" /> BONUS</div><h1 className="text-3xl sm:text-4xl font-extrabold text-white mt-5">Bonus / Próximamente</h1><p className="text-slate-400 mt-2">Nuevos recursos y materiales especiales en preparación.</p><div className="max-w-sm mx-auto mt-8 p-5 rounded-2xl border border-white/10 bg-black/20 text-left"><label className="text-xs font-semibold text-fuchsia-200">Acceso con contraseña</label><Input type="password" value={bonusPassword} onChange={(event) => setBonusPassword(event.target.value)} placeholder="Ingresa tu contraseña" className="mt-2 bg-slate-950/80 border-white/15 text-white" /><Button onClick={() => toast.info("Bonus próximamente", { description: bonusPassword ? "La contraseña será validada cuando se publique el contenido." : "Ingresa la contraseña de acceso." })} className="w-full mt-3 bg-fuchsia-500 hover:bg-fuchsia-400 text-white font-bold">Ingresar al Bonus</Button></div></div></main></div>
    );
  }
  ...
  const openWinolsDownload = () => window.open(WINOLS_MEGA_URL, "_blank", "noopener,noreferrer");
  ...
  const openEcmDownload = () => window.open(ECM_MEGA_URL, "_blank", "noopener,noreferrer");

  const openEcmFile = (file: (typeof ecmTitaniumTree.children)[number]) => {
    window.open(file.webViewLink, "_blank", "noopener,noreferrer");
  };

  const visibleEcmFiles = ecmTitaniumTree.children.filter((file) => file.name.toLowerCase().includes(folderSearch.trim().toLowerCase()));

  const openRemapVideos = () => window.open(remapVideosTree.webViewLink, "_blank", "noopener,noreferrer");

  const openModule6Download = () => window.open(MODULE6_DRIVE_URL, "_blank", "noopener,noreferrer");
  ...
  if (moduleId === "software-remap") {
    return (
      <div className="min-h-screen flex flex-col bg-[#050811] text-slate-100">
        ...
        <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10 flex-1 w-full">
          <Link href="/" className="inline-flex items-center gap-2 text-xs font-semibold text-slate-400 hover:text-white transition-colors"><ArrowLeft className="h-3.5 w-3.5" /> Volver a los módulos</Link>
          <div className="mt-8 mb-6"><div className="flex items-center gap-2 mb-2"><span className="inline-flex items-center gap-2 px-3 py-1 rounded-xl border border-cyan-500/40 bg-cyan-500/10 text-cyan-300 text-[11px] font-bold uppercase tracking-wider"><Box className="h-4 w-4" /> MÓDULO 05</span>{isVip ? <Badge className="bg-emerald-500/10 text-emerald-300 border-emerald-500/30 text-[10px]"><Unlock className="h-3 w-3 mr-1" /> Acceso abierto</Badge> : <Badge className="bg-amber-500/10 text-amber-300 border-amber-500/30 text-[10px]"><Lock className="h-3 w-3 mr-1" /> Acceso abierto</Badge>}</div><h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">Paquete de Software Profesional para Remap</h1><p className="text-sm text-slate-400 font-medium mt-1">Suite completa de herramientas de reprogramación</p></div>
          <section className="p-5 sm:p-7 rounded-2xl border border-white/15 bg-[#07101a]/90 shadow-2xl shadow-cyan-950/20"><p className="text-sm text-slate-300 leading-relaxed">Suite completa con instaladores, activadores y videoclases paso a paso. Todos los enlaces oficiales organizados por programa.</p><div className="mt-5 p-5 rounded-xl border border-fuchsia-400/30 bg-gradient-to-br from-fuchsia-950/45 via-[#180d20]/90 to-[#07101a]/95 shadow-lg shadow-fuchsia-950/20 flex flex-col sm:flex-row sm:items-center justify-between gap-4"><div className="flex items-start gap-3"><div className="h-11 w-11 rounded-xl bg-fuchsia-400/15 border border-fuchsia-300/30 flex items-center justify-center shrink-0"><PlayCircle className="h-5 w-5 text-fuchsia-300" /></div><div><div className="text-[10px] uppercase tracking-widest text-fuchsia-300 font-bold">VIDEOCLASES</div><h2 className="text-lg font-bold text-white mt-1">Videoclases de las instalaciones y paso a paso de los programas</h2><p className="text-xs text-fuchsia-100/70 mt-1">Aprende visualmente cómo instalar y configurar cada uno de los programas del paquete.</p></div></div><Button onClick={openRemapVideos} className={isVip ? "bg-fuchsia-400 hover:bg-fuchsia-300 text-fuchsia-950 font-bold shadow-lg shadow-fuchsia-500/20" : "bg-fuchsia-500 hover:bg-fuchsia-400 text-white font-bold"}>{isVip ? <><PlayCircle className="h-4 w-4 mr-2" /> Acceder a los videos</> : <><Lock className="h-4 w-4 mr-2" /> Acceder a los videos</>}</Button></div></section>
          <section className="mt-6 rounded-2xl border border-amber-400/30 bg-gradient-to-br from-amber-950/30 via-slate-900/80 to-slate-950/80 p-5 sm:p-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4"><div><div className="text-[10px] uppercase tracking-widest text-amber-300 font-bold">ACTIVADORES</div><h2 className="text-xl font-bold text-white mt-1">Todos los Activadores y Generadores de Contraseña</h2><p className="text-sm text-slate-300 mt-2 max-w-2xl">Descarga el archivo completo con todos los activadores y generadores de contraseña del paquete. Si Google Chrome bloquea tu descarga, sigue el paso a paso de las videoclases de abajo.</p></div><Button onClick={() => { if (!isVip) { setLocation("/"); return; } window.location.href = ACTIVATORS_DOWNLOAD; }} className="bg-amber-400 hover:bg-amber-300 text-amber-950 font-bold shrink-0">{isVip ? <><Download className="h-4 w-4 mr-2" /> Descargar archivo de activación</> : <><Lock className="h-4 w-4 mr-2" /> Descargar activadores</>}</Button></div>
          </section>

          <section className="mt-6 rounded-2xl border border-fuchsia-400/30 bg-gradient-to-br from-fuchsia-950/35 via-[#180d20]/80 to-slate-950/80 p-5 sm:p-6"><div className="mb-4"><div className="text-[10px] uppercase tracking-widest text-fuchsia-300 font-bold">VIDEOCLASES — ¿CHROME BLOQUEÓ TU DESCARGA?</div><h2 className="text-xl font-bold text-white mt-1">Aprende a descargar e instalar sin errores</h2></div><div className="grid md:grid-cols-2 gap-4"><div className="rounded-xl overflow-hidden border border-white/10 bg-black"><div className="aspect-video"><iframe className="w-full h-full" src="https://www.youtube.com/embed/CZj9sJvfGBs" title="Videoclase activadores y descargas" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen /></div></div><div className="rounded-xl overflow-hidden border border-white/10 bg-black"><div className="aspect-video"><iframe className="w-full h-full" src="https://www.youtube.com/embed/MqeK4_i3rB4" title="Videoclase solución Chrome" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen /></div></div></div></section>

          <section className="mt-6 rounded-2xl border border-white/10 bg-slate-950/70 p-4 sm:p-5"><div className="mb-4"><div className="text-[10px] uppercase tracking-widest text-cyan-300 font-bold">SOFTWARE INCLUIDO</div><h2 className="text-lg font-bold text-white mt-1">Programas y enlaces del paquete</h2></div><div className="space-y-3">{[
            { title: "Doutor-IE", subtitle: "Contraseña: AUTOTRONICA_LA_PLATA", link: "https://bit.ly/doutor-ie" },
            { title: "Doutor-IE (alt)", subtitle: "Versión alternativa", link: "https://drive.google.com/file/d/1kBuc0PPGgGnKwe-ilMJX9fCZ4n5TJtLq/view" },
            { title: "Simplo", subtitle: "Versión principal", link: "https://drive.google.com/drive/folders/1b5XYcWCGasw8aPglOq7uGi4YYuSMLBlh" },
            { title: "Simplo (alt)", subtitle: "Versión alternativa", link: "https://drive.google.com/file/d/12f9gDbQnbH5FMa0k6st-txZG_OrTT266/view" },
            { title: "Simplo para móvil", subtitle: "Versión optimizada para usar Simplo directamente desde tu dispositivo móvil.", link: "https://drive.google.com/drive/folders/1b5XYcWCGasw8aPglOq7uGi4YYuSMLBlh" },
            { title: "Tecnocar", subtitle: "Programa de reprogramación", link: "https://drive.google.com/drive/folders/1Icbsd0V2MnOslA81c5CFAYwzG_8TeckJ" },
            { title: "Mecánica 2020", subtitle: "Descarga las dos partes y colócalas en la misma carpeta. Contraseña: mecanica84494460", link: "https://drive.google.com/drive/folders/1MUcjeU9Fm0msJwi7LVIO0-jhrNHNYyQW" },
            { title: "Mecánica 2020 (P1)", subtitle: "Primera parte", link: "https://drive.google.com/drive/folders/1MUcjeU9Fm0msJwi7LVIO0-jhrNHNYyQW" },
            { title: "Mecánica 2020 (P2)", subtitle: "Segunda parte", link: "https://drive.google.com/drive/folders/1oVUZXQRmOCh_uLKpObSzF_xLZ-VcBdqh" },
            { title: "Dicatec 3.3", subtitle: "Software incluido", link: "https://drive.google.com/drive/folders/15WBvl93gHVcSTT8K7gvtTyWO-nW8Rk_g" },
            { title: "Adobe Acrobat Reader", subtitle: "Programa obligatorio: Adobe Reader 9.0 o superior para leer los PDF.", link: "https://drive.google.com/file/d/1UHYJexfBYeL8kxZqhmn1UsLNe5aCHE6X/view" },
            { title: "K-Suite (MEGA)", subtitle: "Suite de herramientas", link: "https://mega.nz/file/sLBiSZKA#dvSVEnuCyfrnikDl8_x2BXomPF4-ouAMmqAJdcFakXI" },
          ].map((item) => <div key={item.title} className="rounded-xl border border-white/10 bg-[#080b15] p-3 sm:p-4"><div className="flex items-start justify-between gap-3 mb-2"><div className="min-w-0"><h3 className="font-semibold text-white text-sm">{item.title}</h3><p className="text-[11px] text-slate-400 mt-1">{item.subtitle}</p></div></div><div className="flex flex-wrap gap-2"><Button size="sm" onClick={() => isVip ? window.open(item.link, "_blank", "noopener,noreferrer") : setLocation("/")} className="bg-violet-600 hover:bg-violet-500 text-white font-semibold h-8 text-xs">{isVip ? <><ExternalLink className="h-3.5 w-3.5 mr-1.5" /> {item.title}</> : <><Lock className="h-3.5 w-3.5 mr-1.5" /> Acceso abierto</>}</Button></div></div>)}</div></section>

          <section className="mt-6 rounded-2xl border border-sky-400/25 bg-sky-950/20 p-5 sm:p-6"><div className="mb-4"><div className="text-[10px] uppercase tracking-widest text-sky-300 font-bold">ISAAC REPARADOR AUTOMOTIVO</div><h2 className="text-xl font-bold text-white mt-1">Empieza por aquí ⬇️</h2></div><div className="grid sm:grid-cols-2 gap-3">{[{ title: "Link 1 — SPC MAX MX-0017", link: "https://www.mediafire.com/file/cxf6cz7aksbmbye/SPC_MAX_MX_-_0017.rar/file" }, { title: "Link 2 — Autocom 2021.11", link: "https://www.mediafire.com/file/kgp6modi2wvew96/Autocom_2021.11.rar/file" }, { title: "Link 3 — Delphi 2017 R3", link: "https://bit.ly/3yQwzud" }, { title: "Link 4 — WOW", link: "https://www.mediafire.com/file/3dx1rny9pgn7p4z/WOW_versao_portugues_e_engles.rar/file" }, { title: "Link 5 — Info", link: "https://www.mediafire.com/file/r0o1ghtx7xjptuv/Adobe_e_o_java.rar/file" }].map((item) => <div key={item.title} className="flex items-center justify-between gap-3 rounded-lg border border-white/10 bg-slate-950/60 p-3"><span className="text-sm text-slate-200">{item.title}</span><Button size="sm" variant="outline" onClick={() => isVip ? window.open(item.link, "_blank", "noopener,noreferrer") : setLocation("/")} className="border-sky-400/30 text-sky-300 shrink-0">{isVip ? <><ExternalLink className="h-3.5 w-3.5 mr-1.5" /> Abrir</> : <><Lock className="h-3.5 w-3.5 mr-1.5" /> Abrir</>}</Button></div>)}</div></section>

          <section className="mt-6 rounded-2xl border border-white/10 bg-slate-900/60 overflow-hidden"><div className="p-5 border-b border-white/10 flex items-center justify-between gap-3"><div><div className="text-[10px] uppercase tracking-widest text-cyan-300 font-bold">CARPETA REAL DE GOOGLE DRIVE</div><h2 className="text-base font-bold text-white mt-1">Videoclases</h2><p className="text-xs text-slate-400 mt-1">{remapVideosTree.children.length} carpetas de instalación indexadas.</p></div><Button onClick={() => window.open(remapVideosTree.webViewLink, "_blank", "noopener,noreferrer")} variant="outline" size="sm" className="border-cyan-500/30 text-cyan-300">Abrir en Google Drive <ExternalLink className="h-3.5 w-3.5 ml-1.5" /></Button></div><div className="p-4 space-y-2">{remapVideosTree.children.map((folder) => <button key={folder.id} type="button" onClick={() => window.open(folder.webViewLink, "_blank", "noopener,noreferrer")} className="w-full flex items-center justify-between gap-3 p-3 rounded-lg bg-slate-950/60 border border-white/5 hover:bg-cyan-500/10 text-left transition-colors"><span className="flex items-center gap-2 min-w-0"><FolderSync className="h-4 w-4 text-cyan-400 shrink-0" /><span className="text-sm text-slate-200 truncate">{folder.name}</span></span><ExternalLink className="h-3.5 w-3.5 text-slate-500 shrink-0" /></button>)}</div></section>
        </main>
      </div>
    );
  }
  ...
  if (moduleId === "ecm-titanium") {
    return (
      <div className="min-h-screen flex flex-col bg-[#050811] text-slate-100">
        ...
        <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10 flex-1 w-full">
          <Link href="/" className="inline-flex items-center gap-2 text-xs font-semibold text-slate-400 hover:text-white transition-colors"><ArrowLeft className="h-3.5 w-3.5" /> Volver a los módulos</Link>
          <div className="mt-8 mb-6"><div className="flex items-center gap-2 mb-2"><span className="inline-flex items-center gap-2 px-3 py-1 rounded-xl border border-cyan-500/40 bg-cyan-500/10 text-cyan-300 text-[11px] font-bold uppercase tracking-wider"><Zap className="h-4 w-4" /> MÓDULO 04</span>{isVip ? <Badge className="bg-emerald-500/10 text-emerald-300 border-emerald-500/30 text-[10px]"><Unlock className="h-3 w-3 mr-1" /> Acceso abierto</Badge> : <Badge className="bg-amber-500/10 text-amber-300 border-amber-500/30 text-[10px]"><Lock className="h-3 w-3 mr-1" /> Acceso abierto</Badge>}</div><h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">ECM Titanium</h1><p className="text-sm text-slate-400 font-medium mt-1">Editor alternativo con drivers ilimitados</p></div>
          <section className="p-5 sm:p-7 rounded-2xl border border-fuchsia-400/35 bg-gradient-to-br from-fuchsia-950/45 via-[#180d20]/90 to-[#07101a]/95 shadow-2xl shadow-fuchsia-950/25"><p className="text-sm text-slate-300 leading-relaxed">ECM Titanium es un editor profesional para trabajar con drivers, mapas y compatibilidad de centralitas. Aquí encontrás el instalador completo y las versiones disponibles para descargar.</p><div className="mt-5 p-4 rounded-xl bg-fuchsia-950/35 border border-fuchsia-400/30 flex flex-col sm:flex-row sm:items-center justify-between gap-4"><div><div className="text-[10px] uppercase tracking-widest text-fuchsia-300 font-bold">DESCARGA OFICIAL</div><h2 className="text-lg font-bold text-white mt-1">ECM Titanium — Instalador completo</h2><p className="text-xs text-fuchsia-100/70 mt-1">Archivo alojado en MEGA. Haz clic abajo para descargar el instalador.</p></div><Button onClick={openEcmDownload} className={isVip ? "bg-fuchsia-400 hover:bg-fuchsia-300 text-fuchsia-950 font-bold shadow-lg shadow-fuchsia-500/20" : "bg-fuchsia-500 hover:bg-fuchsia-400 text-white font-bold"}>{isVip ? <><Download className="h-4 w-4 mr-2" /> Descargar ECM Titanium</> : <><Lock className="h-4 w-4 mr-2" /> Descargar</>}</Button></div></section>
          <section className="mt-6 rounded-2xl border border-white/10 bg-slate-900/60 overflow-hidden"><div className="p-5 border-b border-white/10 flex flex-col sm:flex-row sm:items-center justify-between gap-3"><div><div className="text-[10px] uppercase tracking-widest text-cyan-300 font-bold">CARPETA REAL DE GOOGLE DRIVE</div><h2 className="text-base font-bold text-white mt-1">ECM Titanium</h2><p className="text-xs text-slate-400 mt-1">{ecmTitaniumTree.children.length} archivos indexados en esta carpeta.</p></div><Button onClick={() => window.open(ecmTitaniumTree.webViewLink, "_blank", "noopener,noreferrer")} variant="outline" size="sm" className="border-cyan-500/30 text-cyan-300">Abrir en Google Drive <ExternalLink className="h-3.5 w-3.5 ml-1.5" /></Button></div><div className="p-4"><div className="relative mb-3"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-500" /><Input value={folderSearch} onChange={(event) => setFolderSearch(event.target.value)} placeholder="Buscar archivo por nombre..." className="h-10 pl-9 bg-slate-950/70 border-white/10 text-white placeholder:text-slate-500" /></div><div className="rounded-xl border border-white/10 overflow-hidden">{visibleEcmFiles.map((file, index) => <button key={file.id} type="button" onClick={() => openEcmFile(file)} className={`w-full flex items-center justify-between gap-3 p-3 text-left hover:bg-cyan-500/10 transition-colors ${index ? "border-t border-white/10" : ""}`}><span className="flex items-center gap-2 min-w-0"><FileBox className="h-4 w-4 text-cyan-400 shrink-0" /><span className="min-w-0"><span className="block text-sm text-slate-200 truncate">{file.name}</span><span className="block text-[11px] text-slate-500 mt-0.5">{Number(file.size) < 1024 ? `${file.size} B` : `${(Number(file.size) / 1024 / 1024).toFixed(1)} MB`} · {new Date(file.modifiedTime).toLocaleDateString("es-AR")}</span></span></span><span className="flex items-center gap-2 shrink-0"><Badge variant="outline" className="text-[10px] border-cyan-500/20 text-cyan-300">{file.mimeType.includes("rar") ? "RAR" : "TXT"}</Badge>{isVip ? <Download className="h-3.5 w-3.5 text-slate-500" /> : <Lock className="h-3.5 w-3.5 text-amber-400" />}</span></button>)}</div></div></section>
        </main>
      </div>
    );
  }
      ...
      <div className="min-h-screen flex flex-col bg-[#050811] text-slate-100">
        ...
        <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10 flex-1 w-full">
          ...
          <div className="mt-8 mb-6">
            <div className="flex items-center gap-2 mb-2"><span className="inline-flex items-center gap-2 px-3 py-1 rounded-xl border border-cyan-500/40 bg-cyan-500/10 text-cyan-300 text-[11px] font-bold uppercase tracking-wider"><Cpu className="h-4 w-4" /> MÓDULO 03</span>{isVip ? <Badge className="bg-emerald-500/10 text-emerald-300 border-emerald-500/30 text-[10px]"><Unlock className="h-3 w-3 mr-1" /> Acceso abierto</Badge> : <Badge className="bg-amber-500/10 text-amber-300 border-amber-500/30 text-[10px]"><Lock className="h-3 w-3 mr-1" /> Acceso abierto</Badge>}</div>
          ...
          <section className="p-5 sm:p-7 rounded-2xl border border-white/15 bg-[#07101a]/90 shadow-2xl shadow-cyan-950/20">
            ...
            <div className="mt-5 p-4 rounded-xl bg-slate-950/80 border border-cyan-500/20 flex flex-col sm:flex-row sm:items-center justify-between gap-4"><div><div className="text-[10px] uppercase tracking-widest text-cyan-300 font-bold">DESCARGA OFICIAL</div><h2 className="text-lg font-bold text-white mt-1">WinOLS — Instalador completo</h2><p className="text-xs text-slate-400 mt-1">Archivo alojado en Mega para descargar e instalar.</p></div><Button onClick={openWinolsDownload} className={isVip ? "bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold" : "bg-cyan-500 hover:bg-cyan-400 text-slate-950 font-bold"}>{isVip ? <><Download className="h-4 w-4 mr-2" /> Descargar WinOLS</> : <><Lock className="h-4 w-4 mr-2" /> Descargar</>}</Button></div>
          </section>
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10 flex-1 w-full">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-2 mb-2">
            ...
            {isVip ? (
              <Badge className="bg-emerald-500/10 text-emerald-300 border-emerald-500/30 text-[10px]"><Unlock className="h-3 w-3 mr-1" /> Acceso abierto</Badge>
            ) : (
              <Badge className="bg-amber-500/10 text-amber-300 border-amber-500/30 text-[10px]"><Lock className="h-3 w-3 mr-1" /> Acceso abierto</Badge>
            )}
          </div>
        ...
        {isDriveIndexedModule ? (
          ...
          <div className="p-6 sm:p-8 rounded-3xl bg-slate-900/80 border border-white/10 relative overflow-hidden mb-8">
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
              ...
              <Button onClick={handleOpenDrive} className="bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold w-full md:w-auto"><FolderSync className="h-4 w-4 mr-2" /> Abrir Carpeta Completa en Drive</Button>
            </div>
          </div>
        )}
        ...
        {!isDriveIndexedModule && (
          <div className="space-y-4 mt-8">
            ...
            {files.length === 0 ? <div className="p-8 text-center rounded-2xl bg-white/[0.02] border border-white/5"><p className="text-sm text-slate-400">Cuando compartas la carpeta de Drive listaremos aquí todos sus archivos ordenados.</p></div> : <div className="space-y-3">{files.map((file) => <div key={file.id} className="p-4 rounded-xl bg-slate-900/50 border border-white/5 flex flex-col sm:flex-row sm:items-center justify-between gap-4"><div><div className="flex items-center gap-2"><span className="font-semibold text-white text-sm">{file.name}</span><span className="text-[10px] font-bold text-cyan-400 bg-cyan-500/10 px-2 py-0.5 rounded">{file.extension}</span></div><div className="text-xs text-slate-400 flex flex-wrap items-center gap-3 mt-1"><span>Categoría: {file.category}</span>{file.brand && <span>• {file.brand}</span>}{file.ecuType && <span>• {file.ecuType}</span>}<span>• {(file.sizeBytes / (1024 * 1024)).toFixed(1)} MB</span></div></div><Button size="sm" variant="outline" onClick={() => handleDownload(file.name)} className="border-cyan-500/30 text-cyan-300 hover:bg-cyan-500/10"><Download className="h-4 w-4 mr-1.5" /> Descargar</Button></div>)}</div>}
          </div>
        )}
      </main>
    </div>
